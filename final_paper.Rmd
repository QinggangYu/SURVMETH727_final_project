...
The 21st century may be an era of the infectious disease [@quammen2012spillover]. Indeed, the 21st century has been inflicted by various pandemics: acute respiratory syndrome (SARS) in 2003, H1N1 influenza in 2009, Ebola in 2015, and since the end of 2019 till today, COVID-19. Besides imposing enormous threats to people's physical health, pandemics also cast huge influence on mental health. Since its outbreak, COVID-19 has brought grave consequences for people's emotion and psychological well-being, regardless of age or gender[@Philip2020; @Milne2020; @Phelps2020; @Zhang2020; @Terry2020; @Kumar2020; @Pfefferbaum2020]. Therefore, understanding how pathogen threat (like the current pandemic) shifts people's emotional patterns or sentiments presents as a timely investigation.   

However, one cannot fully understand the sentiment pattern of any group or society without understanding its cultural underpinning, given the profound way culture shapes one's emotion [@tsai2006cultural; @tsai2007ideal]. One important dimension that cultures differ on is the dimension of collectivism - individualism [@Markus1991; @hofstede1984culture]. Whereas individualistic cultures emphasize freedom, personal choice, self-interest, personal achievements, and the agency and independence of the self, collectivist cultures emphasize social roles, obligations, social norms, and the importance of groups, relationships, and hierarchy.  

The construct of collectivism-individualism is particularly relevant to the discussion of pathogen threat because prior work has proposed that variation in historical pathogen prevalence across of the globe is the very reason that cultures differ on this dimension [@Fincher2008]. Throughout the human history, individuals, as well as social groups need to possess psychological mechanisms that serve the function of antipathogen defense in order to survive, if they live in regions with high pathogen prevalence. One effective antipathogen defense mechanism is collectivism. First, collectivists make clear distinction between ingroup and outgroup, and are more likely to show ingroup favoritism and outgroup denigration [@house2004culture]. They are thus more cautious when being around of outgroup, who, in human history, are more likely to carry novel pathogens [@diamond1999guns]. Second, collectivist cultures have stronger social norms on individual's behaviors [@gelfand2006nature], including many that has direct consequences on antipathogen defense [@sherman1999darwinian]. Therefore, it stands to reason that people from collectivist cultures should be more wary of pathogen threat, and hence showing a more negative sentiment in response to pathogen threat.  

However, one may hypothesize the exact opposite. Collectivism comes with a greater range of and more interconnected relationships. Social relationships, especially close ones, are typically considered as warm and soothing [@williams2008experiencing]. For instance, it has been shown that physical reactions to pain are attentuated in the presence of close others [@coan2006lending]. Similarly, people who are more socially connected show reduced physiological response to physical pain [@Eisenberger2007]. Extrapolating from these findings, it is possible that when feeling embedded in social relationships, it may generate a so-called "analgesic effect", so as individuals become less sensitive and vigilant to the danger and threat in the surrounding environment. Indeed, a recent study shows that when primed with pathogen threat, individuals who are more interdependent (or collectivist) become less vigilant to norm violations [@Salvador2020]. Therefore, an alternative hypothesis is that people from collectivist cultures may in fact be less sensitive to pathogen threat, and thus exhibit a more positive (or less negative) sentiment towards the pandemic.

To test these competing hypotheses, we investigate people's expression of their sentiments towards the pandemic as they naturally occur. In particular, we prospectly collected posts (or tweets) from Twitter, one of the most popular social media, and analyzed the sentiments. This method enables the collection of a large amount of data which are not limited by geographical constraint. We focused our investigation on each of the states of the USA, which has been shown to differ in the degree of collectivism. We examined whether the tweets coming from different states differ in the sentiments, as a function of the level of collectivism of each state.
...
All RDS files were read in, time of data collection was properly labeled and a unique ID was created for each tweet before collapsed into one dataframe. To reduce the size of the dataframe, only relevant variables were kept. 

Finally, because the data collection time overlapped with the U.S. president election time of 2020, we labeled and removed tweets that were politically charged by using the following keywords.
...
## Covid-related tweet-streaming in python
Below is the full python codes for collecting covid-related tweets (using keywords in the query) while at the same time limiting the collection within the U.S. bound box.
...
## General tweet-streaming in R
Below is the full R codes for collecting general tweets in R. The specific time of data collection on each day is shown in the comment.

```{r tweet streaming, eval=FALSE}
library(rtweet)

# Day 1 - 10/30/2020, pm only
## start from 06:30 PM, EST 10/30/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201030_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day1pm  <- parse_stream("COVID_III_20201030_pm.json")
saveRDS(COVID_III_day1pm, "COVID_20201030_pm.rds")
...
# Day 3 - 11/01/2020, am & pm
## start from 9:30 AM, EST 11/01/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201101_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day3am  <- parse_stream("COVID_III_20201101_am.json")
saveRDS(COVID_III_day3am, "COVID_20201101_am.rds")

## start from 9:05 PM, EST 11/01/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201101_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day3pm  <- parse_stream("COVID_III_20201101_pm.json")
saveRDS(COVID_III_day3pm, "COVID_20201101_pm.rds")

# Day 4 - 11/02/2020, am & pm
## start from 9:00 AM, EST 11/02/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201102_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day4am  <- parse_stream("COVID_III_20201102_am.json")
saveRDS(COVID_III_day4_am, "COVID_20201102_am.rds")

# start from 11:55 PM, EST 11/02/2020, json file broken as well 
# (originally planned for 8:00 pm, json file broken)
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201102_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day4pm  <- parse_stream("COVID_III_20201102_pm.json")
saveRDS(COVID_III_day4pm, "COVID_20201102_am.rds")

# Day 5 - 11/03/2020, am & pm
## start from 9:30 AM, EST 11/03/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201103_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day5am  <- parse_stream("COVID_III_20201103_am.json")
saveRDS(COVID_III_day5am, "COVID_20201103_am.rds")

# start from 8:00 PM, EST 11/03/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201103_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day5pm  <- parse_stream("COVID_III_20201103_pm.json")
saveRDS(COVID_III_day5pm, "COVID_20201103_pm.rds")

# Day 6 - 11/04/2020, am & pm
## originally start from 10:30 AM, EST 11/04/2020
## yet, the stream broke around 11:20 AM.
## start from 11:30 AM, EST 11/04/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201104_am.json",
  language = "en",
  parse = FALSE
)
COVID_III_day6am  <- parse_stream("COVID_III_20201104_am.json")

## start from 8:08 PM, EST 11/04/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201104_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day6pm  <- parse_stream("COVID_III_20201104_pm.json")

# Day 7 - 11/05/2020, am & pm
## start from 10:30 AM, EST 11/05/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201105_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day7am  <- fromJSON(readLines("COVID_III_20201105_am.json"))

## start from 7:05 PM, EST 11/05/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201105_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day7pm  <- parse_stream("COVID_III_20201105_pm.json")
saveRDS(COVID_III_day7pm, "COVID_20201105_pm.rds")

# Day 8 - 11/06/2020, am & pm
## start from 10:00 AM, EST 11/06/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201106_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day8am  <- parse_stream("COVID_III_20201106_am.json")
saveRDS(COVID_III_day8am, "COVID_20201106_am.rds")

## start from 8:30 PM, EST 11/06/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201106_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day8pm  <- parse_stream("COVID_III_20201106_pm.json")
saveRDS(COVID_III_day8pm, "COVID_20201106_pm.rds")

# Day 9 - 11/07/2020, am & pm
## start from 11:30 AM, EST 11/07/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201107_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day9am  <- parse_stream("COVID_III_20201107_am.json")
saveRDS(COVID_III_day9am, "COVID_20201107_am.rds")

## start from 6:10 PM, EST 11/07/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201107_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day9pm  <- parse_stream("COVID_III_20201107_pm.json")
saveRDS(COVID_III_day9pm, "COVID_20201107_pm.rds")

# Day 10 - 11/08/2020, am & pm
## start from 10:00 AM, EST 11/08/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201108_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day10am  <- parse_stream("COVID_III_20201108_am.json")
saveRDS(COVID_III_day10am, "COVID_20201108_am.rds")

## start from 8:00 PM, EST 11/08/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201108_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day10pm  <- parse_stream("COVID_III_20201108_pm.json")
saveRDS(COVID_III_day10pm, "COVID_20201108_pm.rds")

# Day 11 - 11/09/2020, am & pm
## start from 9:40 AM, EST 11/09/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201109_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day11am  <- parse_stream("COVID_III_20201109_am.json")
COVID_III_day11am <- fromJSON(read_lines("COVID_III_20201109_am.json"))


## start from 7:30 PM, EST 11/09/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201109_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day11pm  <- parse_stream("COVID_III_20201109_pm.json")
saveRDS(COVID_III_day11pm, "COVID_20201109_pm.rds")

# Day 12 - 11/10/2020, am & pm
## start from 11:50 AM, EST 11/10/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201110_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day12am  <- parse_stream("COVID_III_20201110_am.json")
saveRDS(COVID_III_day12am, "COVID_20201110_am.rds")


## start from 8:00 PM, EST 11/10/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201110_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day12pm  <- parse_stream("COVID_III_20201110_pm.json")
saveRDS(COVID_III_day12pm, "COVID_20201110_pm.rds")

# Day 13 - 11/11/2020, am & pm
## start from 10:00 AM, EST 11/11/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201111_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day13am  <- parse_stream("COVID_III_20201111_am.json")
saveRDS(COVID_III_day13am, "COVID_20201111_am.rds")

## start from 8:30 PM, EST 11/11/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201111_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day13pm  <- parse_stream("COVID_III_20201111_pm.json")
saveRDS(COVID_III_day13pm, "COVID_20201111_pm.rds")

# Day 14 - 11/12/2020, am & pm
## start from 12:30 PM, EST 11/12/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201112_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day14am  <- parse_stream("COVID_III_20201112_am.json")
saveRDS(COVID_III_day14am, "COVID_20201112_am.rds")


# start from 6:30 PM, EST 11/12/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201112_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day14pm  <- parse_stream("COVID_III_20201112_pm.json")
saveRDS(COVID_III_day14pm, "COVID_20201112_pm.rds")

# Day 15 - 11/13/2020, am & pm
## start from 9:30 AM, EST 11/13/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201113_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day15am  <- parse_stream("COVID_III_20201113_am.json")
saveRDS(COVID_III_day15am, "COVID_20201113_am.rds")

## start from 9:00 PM, EST 11/13/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201113_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day15pm  <- parse_stream("COVID_III_20201113_pm.json")
saveRDS(COVID_III_day15pm, "COVID_20201113_pm.rds")

# Day 16 - 11/14/2020, am & pm
## start from 9:00 AM, EST 11/14/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201114_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day16am  <- parse_stream("COVID_III_20201114_am.json")
saveRDS(COVID_III_day16am, "COVID_20201114_am.rds")

# start from 9:15 PM, EST 11/14/2020  - MISSING
# originally stored in the wrong file 20201113_pm.json
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201114_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day16pm  <- parse_stream("COVID_III_20201114_pm.json")
saveRDS(COVID_III_day16pm, "COVID_20201114_pm.rds")

# Day 17 - 11/15/2020, am & pm
## start from 10:00 AM, EST 11/15/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201115_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day17am  <- parse_stream("COVID_III_20201115_am.json")
saveRDS(COVID_III_day17am, "COVID_20201115_am.rds")

## start from 7:00 PM, EST 11/15/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201115_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day17pm  <- parse_stream("COVID_III_20201115_pm.json")
saveRDS(COVID_III_day17pm, "COVID_20201115_pm.rds")

# Day 18 - 11/16/2020, am & pm
## start from 11:30 AM, EST 11/16/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201116_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day18am  <- parse_stream("COVID_III_20201116_am.json")
saveRDS(COVID_III_day18am, "COVID_20201116_am.rds")

## start from 6:30 PM, EST 11/16/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201116_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day18pm  <- parse_stream("COVID_III_20201116_pm.json")
saveRDS(COVID_III_day18pm, "COVID_20201116_pm.rds")

# Day 19 - 11/17/2020, am & pm
## start from 9:30 AM, EST 11/17/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201117_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day19am  <- parse_stream("COVID_III_20201117_am.json")
saveRDS(COVID_III_day19am, "COVID_20201117_am.rds")


## start from 8:30 PM, EST 11/17/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201117_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day19pm  <- parse_stream("COVID_III_20201117_pm.json")
saveRDS(COVID_III_day19pm, "COVID_20201117_pm.rds")

# Day 20 - 11/18/2020, am & pm
## start from 11:45 AM, EST 11/18/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201118_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day20am  <- parse_stream("COVID_III_20201118_am.json")
saveRDS(COVID_III_day20am, "COVID_20201118_am.rds")

## start from 8:30 PM, EST 11/18/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201118_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day20pm  <- parse_stream("COVID_III_20201118_pm.json")
saveRDS(COVID_III_day20pm, "COVID_20201118_pm.rds")

# Day 21 - 11/19/2020, am & pm
## start from 9:30 AM, EST 11/19/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201119_am.json",
  language = "en",
  parse = FALSE
)

COVID_III_day21am  <- parse_stream("COVID_III_20201119_am.json")
saveRDS(COVID_III_day21am, "COVID_20201119_am.rds")


# start from 6:00 PM, EST 11/18/2020
stream_tweets(
  lookup_coords("usa"),
  timeout = 60*60*3,  #live streaming for 3hrs
  file_name = "COVID_III_20201119_pm.json",
  language = "en",
  parse = FALSE
)

COVID_III_day21pm  <- parse_stream("COVID_III_20201119_pm.json")
saveRDS(COVID_III_day21pm, "COVID_20201119_pm.rds")
```
